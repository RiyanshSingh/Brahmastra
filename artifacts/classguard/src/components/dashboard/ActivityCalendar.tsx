...
// Static mock for aesthetics
const DAYS = ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'];
const DATES = Array.from({ length: 35 }, (_, i) => {
  const day = i - 2; // Offset to start month
  if (day <= 0) return { date: 30 + day, current: false };
  if (day > 31) return { date: day - 31, current: false };
  
  // Randomly assign activity types for aesthetic
  let activity = null;
...
export function ActivityCalendar() {
  return (
    <motion.div 
      ...
      className="glass-card rounded-[2rem] p-6 shadow-sm"
    >
      <div className="flex items-center justify-between mb-6">
        <h3 className="font-semibold text-lg text-foreground">October 2024</h3>
        <div className="flex gap-2">
          <button className="p-1.5 rounded-full hover:bg-slate-100 text-muted-foreground transition-colors">
        ...
        {DATES.map((d, i) => (
          <div key={i} className="flex justify-center relative">
            <div className={cn(
              ...
              d.current && !d.isToday && "text-foreground hover:bg-slate-100",
