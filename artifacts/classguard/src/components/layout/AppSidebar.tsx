...
import {
  ...
  Shield, Settings, HelpCircle, LogOut, Smartphone
} from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { label: "Student Mark", href: "/mark-attendance", icon: Smartphone },
];
...
export function AppSidebar({
  ...
  return (
    <aside className={cn(
      "w-[220px] shrink-0 h-screen sticky top-0 flex flex-col bg-[hsl(var(--sidebar))] border-r border-[hsl(var(--sidebar-border))]",
      className,
    )}>
      {/* Logo */}
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          ...
          return (
            <Link key={label} href={href}>
              ...
              >
                <Icon className={cn(
                  "w-4 h-4 shrink-0",
                  isActive ? "text-[hsl(var(--sidebar-primary))]" : "text-muted-foreground/80"
                )} />
                {label}
              </div>
            </Link>
        ...
        <div className="pt-4">
          ...
          {SECONDARY_ITEMS.map(({ label, href, icon: Icon }) => (
            <Link key={label} href={href}>
              <div
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:bg-[hsl(var(--sidebar-accent))] hover:text-[hsl(var(--sidebar-foreground))] transition-all duration-150 cursor-pointer select-none"
                onClick={onNavigate}
              >
                <Icon className="w-4 h-4 shrink-0 text-muted-foreground/80" />
                {label}
              </div>
            </Link>
          ))}
      ...
      <div className="px-3 pb-4 border-t border-[hsl(var(--sidebar-border))] pt-3">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[hsl(var(--sidebar-accent))] transition-colors cursor-pointer">
          ...
          <LogOut className="w-3.5 h-3.5 text-muted-foreground/80 shrink-0" />
        </div>
      </div>
    </aside>
  );
